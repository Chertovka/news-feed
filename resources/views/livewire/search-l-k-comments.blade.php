    ...
    <table class="min-w-full">
        ...
        <tr>
            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                <a href="{{ route('lk.comments.index', ['sort' => 'id', 'page' => Request()->page, 'order' => toggleOrder('id')]) }}">
                    ID </a>
            </th>
            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                <a href="{{ route('lk.comments.index', ['sort' => 'text', 'page' => Request()->page, 'order' => toggleOrder('text')]) }}">
                    Текст
                </a>
            </th>
            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                <a href="{{ route('lk.comments.index', ['sort' => 'post_id', 'page' => Request()->page, 'order' => toggleOrder('post_id')]) }}">
                    Название поста</a>
            </th>
        ...
        @foreach($comments as $comment)
            <tr wire:key="{{ $comment->id }}">
                ...
                <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                    <a href="{{ route("lk.comments.edit", $comment->id) }}"
                       class="text-indigo-600 hover:text-indigo-900">Редактировать</a>
                    <form action="{{ route("lk.comments.destroy", $comment->id) }}" method="POST">
                        @csrf
